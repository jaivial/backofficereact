...
export default function Page() {
  ...
  return (
    <section className="p-6" aria-label="Categorias de comida">
        ...
        <div className="grid grid-cols-4 grid-gap-3" role="list">
          {FOOD_ENTRIES.map((entry) => {
            ...
            return (
              ...
              >
                <EntryIcon className="text-bo-accent" size={20} aria-hidden="true" />
                <span className="text-bo-text text-base font-bold leading-tight">{entry.label}</span>
                <span className="text-bo-muted text-xs leading-normal">{entry.hint}</span>
              </button>
            );
          })}
        </div>
      ...
      <button className="fixed bottom-8 right-8 w-14 h-14 rounded-full bg-bo-accent text-bo-bg shadow-soft flex items-center justify-center cursor-pointer transition-transform duration-150 transition-shadow duration-150 border-none hover:scale-105 hover:shadow-lg" type="button" aria-label="Crear elemento de comida" onClick={openCreate}>
        <Plus size={26} />
      </button>
    </section>
  );
