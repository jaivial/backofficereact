...
export function CashControl({ onChanged }: { onChanged?: () => void | Promise<void> }) {
  ...
  return <section className="grid gap-4" data-ui="pos-cash-control">
    ...
    <article className="rounded-xl border border-[var(--bo-border)] bg-[var(--bo-surface)] p-4" data-ui="pos-cash-history"><div className="flex items-center justify-between"><h3 className="font-semibold text-[var(--bo-text)]">Histórico de cierres</h3>{lastClosure ? <button className="min-h-11 rounded-lg border border-[var(--bo-border-2)] px-3 text-[var(--bo-accent)]" type="button" onClick={() => void pdf(lastClosure)} data-ui="pos-cash-last-pdf">PDF último</button> : null}</div><div className="mt-3 grid gap-2">{closures.map((closure) => <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[var(--bo-border)] p-3 text-sm" key={closure.id} data-ui="pos-cash-closure-row"><span className="text-[var(--bo-text)]">Cierre {closure.closureType} · turno {closure.shiftId} · {new Date(closure.generatedAt).toLocaleString("es-ES")}</span><span className="flex items-center gap-2"><strong className="text-[var(--bo-accent)]" data-testid={`pos-cash-closure-difference-${closure.id}`}>{closure.differenceCents == null ? "—" : money(closure.differenceCents)}</strong><button className="min-h-11 rounded-lg border border-[var(--bo-border-2)] px-3 text-[var(--bo-accent)]" type="button" onClick={() => void pdf(closure)} data-ui="pos-cash-closure-pdf">PDF</button></span></div>)}</div></article>
  </section>;
}
