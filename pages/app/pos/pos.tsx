...
export default function Page() {
  ...
  // force is threaded API → hook → here; the unclosed-days gate uses it to
  // override the backend's 409 UNCLOSED_PREVIOUS_DAYS refusal.
  const openDay = useCallback(async (openingCashCents: number, force?: boolean) => cashDayState.openDay({ openingCashCents, force }), [cashDayState.openDay]);
  ...
  return <main className="w-full overflow-y-auto" data-ui="pos-page">
    ...
    <POSCalendarModal open={calendarOpen} onClose={() => setCalendarOpen(false)} activeDate={activeDate ?? ""} onChangeDate={pickDate} />
