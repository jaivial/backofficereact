...
import { fallbackRoleImportance, fullName, sortRoles } from "./helpers";
...
export default function RolesPage() {
  ...
  return (
    <section aria-label="Roles" className="bo-content-grid bo-membersPage" data-ui="roles-page">
      <div className="bo-panel" data-ui="roles-intro-panel">
        <div className="bo-panelHead bo-membersIntroHead" data-ui="roles-intro-header">
          <div data-ui="roles-intro-text">
            <div className="bo-panelTitle" data-ui="roles-intro-title">Roles y jerarquía</div>
            <div className="bo-panelMeta" data-ui="roles-intro-meta">Gestiona roles por importancia (0-100), permisos y asignación de usuarios.</div>
          </div>
          <div className="bo-membersIntroBadge" data-ui="roles-intro-badge">
            <ShieldUser size={16} strokeWidth={1.8} aria-hidden="true" data-ui="roles-badge-icon" />
            <span data-ui="roles-badge-count">{roles.length} roles</span>
          </div>
        </div>
      </div>

      <div className="bo-roleToolbar" data-ui="roles-toolbar">
        <div className="bo-mutedText" data-ui="roles-actor-importance">
          Tu importancia actual: {actorImportance}
        </div>
        <button
          className="bo-btn bo-btn--primary bo-btn--sm"
          type="button"
          onClick={() => setCreateOpen(true)}
          data-ui="roles-new-btn"
        >
          <Plus size={14} strokeWidth={1.8} aria-hidden="true" data-ui="roles-new-btn-icon" />
          <span data-ui="roles-new-btn-label">Nuevo rol</span>
        </button>
      </div>

      <div className="bo-roleGrid" role="list" data-ui="roles-grid">
        {roles.map((role) => (
          <RoleCard
            key={role.slug}
            role={role}
