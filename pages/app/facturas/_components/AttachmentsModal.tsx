...
function getFileIconColor(mimeType: string): string {
  if (mimeType.startsWith("image/")) return "var(--text-info)";
  if (mimeType === "application/pdf") return "var(--text-danger)";
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
        <div className="rounded-lg bg-card shadow-soft max-w-lg w-full bo-attachmentsModal" role="dialog" aria-label="Adjuntos de factura" onClick={(e) => e.stopPropagation()}>
      ...
      {previewingAttachment && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setPreviewingAttachment(null)}>
          <div className="rounded-lg bg-card shadow-soft max-w-lg w-full bo-previewModal" role="dialog" aria-label="Vista previa" onClick={(e) => e.stopPropagation()}>
            <div className="bo-previewModalHeader">
              <h3>{previewingAttachment.original_name}</h3>
              <button className="inline-flex h-8 items-center justify-center gap-2 rounded-lg border border-white/[0.06] bg-transparent text-foreground text-xs font-bold transition-all hover:bg-white/[0.04]" onClick={() => setPreviewingAttachment(null)} aria-label="Cerrar">
                <X size={18} />
              </button>
            </div>
