...
import {
	tableSheetViewAtom,
	selectedTableCardIdAtom,
} from "../../../../state/tableManagerAtoms";
...
import type {
	TableMapLayoutTemplate,
	TableMapTemplateScope,
} from "../../../../api/types";
...
function rectsOverlap(
	a: RectSize & XYPosition,
	b: RectSize & XYPosition,
): boolean {
	return (
		a.x < b.x + b.width &&
		a.x + a.width > b.x &&
...
function tableSizeFitsOtherTables(
	...
	return nodes.every((other) => {
		...
		const otherGeom = previewGeometry(
			...
			typeof otherData.width === "number" ||
				typeof otherData.height === "number"
				? { width: otherData.width, height: otherData.height }
				: undefined,
		);
		const otherRotation = Number.isFinite(otherData.rotationDeg)
			? otherData.rotationDeg
			: 0;
		const otherFrame = rotatedRectFrameFromPosition(
			other.position,
			otherGeom.width,
			otherGeom.height,
			otherRotation,
...
export function GuestNamesModal({
	tableName,
	capacity,
	names,
	onSave,
	onClose,
}: GuestNamesModalProps) {
	...
	return (
		<div data-ui="guest-names-modal" className="bo-guestNamesModal">
			<div
				data-ui="guest-names-backdrop"
				className="bo-guestNamesBackdrop"
				onClick={onClose}
			/>
...
async function downloadMultiTablePdf(
	...
	for (const assignment of assignments) {
		...
		const cardHeight = Math.max(
			CARD_MIN_HEIGHT,
			HEADER_HEIGHT + nameCount * LINE_HEIGHT + 10,
		);
...
function BookingAssignmentEditor({
	booking,
	state,
	tables,
	occupiedSeats,
	onSave,
}: BookingAssignmentEditorProps) {
	...
	const optionsForRow = useCallback(
		(row: BookingTableAssignment): Array<{ value: string; label: string }> => {
			...
			const exists = tableOptions.some(
				(o) => normalizeTableKey(o.value) === normalizeTableKey(row.table_name),
			);
			if (exists) return tableOptions;
			return [
				{ value: row.table_name, label: `${row.table_name} (actual)` },
				...tableOptions,
			];
		},
		[tableOptions],
	...
	const handleTableSelect = useCallback(
		(idx: number, tableName: string) => {
			const table = tables.find(
				(t) => normalizeTableKey(t.name) === normalizeTableKey(tableName),
			);
	...
	const addRow = useCallback(() => {
		setDraft((prev) => [
			...prev,
			{ table_id: null, table_name: "", seats: 1, names: [] },
		]);
	}, []);
	...
	const handleDownloadPdf = useCallback(async () => {
		const bookingName =
			booking.customer_name?.trim() || `Reserva ${booking.id}`;
	...
	const modalCapacity = modalRow
		? tableCapacityMap.get(normalizeTableKey(modalRow.table_name)) ||
			modalRow.seats
		: 1;

	return (
		<div
			data-ui="booking-assignment-editor"
			className="bo-bookingAssignmentEditor"
		>
			{draft.length === 0 ? (
				<div data-ui="assignment-empty" className="bo-bookingAssignmentEmpty">
					Sin mesas asignadas
				</div>
			) : (
				<div data-ui="assignment-rows" className="bo-bookingAssignmentRows">
					{draft.map((row, idx) => {
						const rowCapacity =
							tableCapacityMap.get(normalizeTableKey(row.table_name)) ||
							row.seats;
						...
						return (
							<div
								key={idx}
								data-ui="assignment-row"
								className="bo-bookingAssignmentRow"
							>
								<Select
								...
								{isLocked ? (
									<span
										data-ui="assignment-seats-chip"
										className="bo-bookingAssignmentSeatsChip"
										title="Capacidad fija de la mesa"
									>
										{rowCapacity} pax
									</span>
								) : (
									...
									>
										<button
											...
											onClick={() =>
												setRow(idx, { seats: Math.max(1, row.seats - 1) })
											}
										>
											<Minus size={12} strokeWidth={2.2} />
										</button>
										<span
											data-ui="assignment-seats-value"
											className="bo-bookingAssignmentSeatsValue"
										>
											{row.seats}
										</span>
										<button
			...
			<div data-ui="assignment-footer" className="bo-bookingAssignmentFooter">
				...
				<span
					data-ui="assignment-total"
					className={`bo-bookingAssignmentTotal${seatsOk ? " is-ok" : " is-warn"}`}
				>
...
export default function TableManagerPage() {
	...
	const [draggingLimitVertexIndex, setDraggingLimitVertexIndex] = useState<
		number | null
	>(null);
	...
	const [bookingForAssignment, setBookingForAssignment] =
		useState<Booking | null>(null);
	const [assignMode, setAssignMode] = useState(false);
	const [multiTableMode, setMultiTableMode] = useState(false);
	const [multiTableDraft, setMultiTableDraft] = useState<
		BookingTableAssignment[]
	>([]);
	const [multiTableNamesModalIdx, setMultiTableNamesModalIdx] = useState<
		number | null
	>(null);
	...
	const [selectedDrawElementId, setSelectedDrawElementId] = useState<
		string | null
	>(null);
	...
	const [floorTemplate, setFloorTemplate] =
		useState<TableMapLayoutTemplate | null>(null);
	const [templateScope, setTemplateScope] =
		useState<TableMapTemplateScope>("day");
	...
	const [shortSideHover, setShortSideHover] = useState<RectShortSide | null>(
		null,
	);
	const nodeTypes = NODE_TYPES;
	const reduceMotion = useReducedMotion();
	const isDayOpen = day?.isOpen !== false;
	const dayVisibilityTransition = reduceMotion
		? { duration: 0 }
	...
	const mapHistoryRef = useRef(
		new Map<
			string,
			{
				...
				last: MapEditSnapshot | null;
			}
		>(),
	);
	const applyingHistoryRef = useRef(false);
	const drawPanelHoverTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	...
	const saveBookingAssignmentsRef = useRef<
		(booking: Booking, assignments: BookingTableAssignment[]) => Promise<void>
	>(async () => undefined);
	const saveTableSizeRef = useRef<
		(id: string, width: number, height: number) => void
	>(() => undefined);
	const saveDrawElementSizeRef = useRef<
		(id: string, width: number, height: number) => void
	>(() => undefined);
	const geom = useMemo(
	...
	const shortSideCount = useMemo(
		() =>
			Number(normalizedRectShortSides.left) +
			Number(normalizedRectShortSides.right),
		[normalizedRectShortSides.left, normalizedRectShortSides.right],
	);
	const shortSideMax = useMemo(
		() => maxRectShortSeatsForCapacity(draft.capacity),
		[draft.capacity],
	);
	const canAddLeftShortSide =
		isRectangularPreview &&
		!normalizedRectShortSides.left &&
		shortSideCount < shortSideMax;
	const canAddRightShortSide =
		isRectangularPreview &&
		!normalizedRectShortSides.right &&
		shortSideCount < shortSideMax;
	...
	const mapHistoryKey = useCallback(
		...
		[
			pageContext.bo?.session?.activeRestaurantId,
			selectedDate,
			selectedFloor,
			templateScope,
		],
	...
	const setMapHistoryBaseline = useCallback(
		(elements: DrawElement[], points: LinePoint[]) => {
			const history = getMapHistory();
			history.last = cloneMapEditSnapshot({
	...
	const mutateShortSide = useCallback(
		(side: RectShortSide, action: "add" | "remove") => {
			setDraft((prev) => {
				const normalized = normalizeRectShortSides(
					prev.capacity,
					prev.rectShortSides,
				);
				...
				if (action === "add") {
					if (normalized[side] || currentCount >= max)
						return { ...prev, rectShortSides: normalized };
					const next = { ...normalized, [side]: true };
					return {
						...prev,
						rectShortSides: normalizeRectShortSides(prev.capacity, next),
					};
				}
	...
	const onAddShortSide = useCallback(
		(side: RectShortSide) => {
			const canAdd =
				side === "left" ? canAddLeftShortSide : canAddRightShortSide;
			if (!canAdd) return;
	...
	const visibleAreas = useMemo(
		() => floorAreas.get(selectedFloor) || [],
		[floorAreas, selectedFloor],
	);
	const visibleTables = useMemo(
		() => visibleAreas.flatMap((a) => a.tables || []),
		[visibleAreas],
	);
	...
	const hasUnassignedBookings = useMemo(
		() => bookings.some((b) => !b.table_number),
		[bookings],
	);
	...
	const nextTableNumero = useMemo(() => {
		const allTables = areas.flatMap((a) => a.tables || []);
		const used = new Set(
			allTables.map((t) => normalizeTableKey(String(t.numero_mesa ?? ""))),
		);
		let n = allTables.length + 1;
	...
	const occupancy = useMemo(() => {
		...
		const percent =
			limit > 0
				? Math.max(0, Math.min(100, Math.round((totalPeople / limit) * 100)))
				: 0;
	...
	const loadData = useCallback(async () => {
		...
		try {
			...
			if (!tablesRes.success) {
				...
				loadedAreas = (tablesRes.areas || tablesRes.data || []).map((a: any) =>
					normalizeTableArea(a),
				);
				setAreas(loadedAreas);
				const mapLayout = ((tablesRes.layout as any)?.map ||
					(tablesRes.layout as any) ||
				...
				const overrideElements = normalizeLayoutElements(
					mapLayout._draw_elements_template_override,
				);
				const templateElements = normalizeLayoutElements(
					mapLayout.draw_elements_template,
				);
				const loadedElements =
					perDayElements.length > 0
						? perDayElements
				...
				const overrideLimitPoints = normalizeLimitPoints(
					mapLayout._limit_area_template_points_override,
				);
				const templateLayoutPoints = normalizeLimitPoints(
					mapLayout.limit_area_template_points,
				);
				const legacyTemplatePoints = limitAreaTemplatePointsForFloor(
					loadedAreas,
					selectedFloor,
				);
				...
				const history = getMapHistory();
				if (!history.last)
					setMapHistoryBaseline(loadedElements, activeLimitPoints);
				...
				if (mapLayout.template !== undefined) {
					...
					setTemplateScope(
						defaultScope(
							...
							mapLayout.scope as TableMapTemplateScope | undefined,
						),
					);
				}

				const loadedBookingStates: Record<string, BookingState> = {};
				const rawBookingStates = mapLayout.booking_states as
	...
	useEffect(() => {
		...
		async function loadTemplateForFloor() {
			try {
				...
				const tpl = isNonEmptyTemplate((res as any).template)
					? ((res as any).template as TableMapLayoutTemplate)
					: null;
		...
		for (const booking of bookings) {
			...
			for (const assignment of assignments) {
				...
				const seats = Math.max(
					0,
					Number(assignment.seats) || Number(booking.party_size) || 0,
				);
	...
	const getTableBookings = useCallback(
		(tableName: string): Booking[] => {
	...
	const occupiedSeatsExcludingBooking = useCallback(
		(booking: Booking | null): Map<string, number> => {
			...
			const prev = resolveAssignments(
				bookingStates[String(booking.id)],
				booking.table_number,
				booking.party_size,
			);
			for (const a of prev) {
				...
				map.set(
					key,
					Math.max(0, (map.get(key) || 0) - Math.max(0, Number(a.seats) || 0)),
				);
			}
			return map;
		},
	...
	const [selectedTableCardId, setSelectedTableCardId] = useAtom(
		selectedTableCardIdAtom,
	);
	...
	const assignBookingToFreeTable = useCallback(
		async (booking: Booking, tableName: string) => {
			...
			const existing = resolveAssignments(
				bookingStatesRef.current[String(booking.id)],
				booking.table_number,
				booking.party_size,
			);
			const already = existing.some(
				(a) => normalizeTableKey(a.table_name) === normalizeTableKey(tableName),
			);
	...
	const openEditModal = useCallback((table: TableMapItem) => {
		...
		setDraft({
			...
			rectShortSides: shortSidesFromMetadata(
				metadata.short_side_seats,
				capacity,
			),
		});
	 ...
	 */
	const updateLiveTableSize = useCallback(
	...
	const updateLiveDrawElementSize = useCallback(
		(id: string, width: number, height: number) => {
	...
	useEffect(() => {
		setNodes([
			...visibleTables.map((table) => {
				...
				const multiDraftIdx = multiTableDraft.findIndex(
					(d) => d.table_id === table.id,
				);
				...
				return {
					...
					data: {
						...
						onResize: (width, height) =>
							updateLiveTableSize(String(table.id), width, height),
						onResizeEnd: (width, height) =>
							saveTableSizeRef.current(String(table.id), width, height),
						onEditClick:
							editMode && !assignMode ? () => openEditModal(table) : undefined,
						onDeleteClick:
							editMode && !assignMode
								? () => {
						...
						onMultiNamesClick: isMultiSelected
							? () => setMultiTableNamesModalIdx(multiDraftIdx)
							: undefined,
						onMultiRemoveClick: isMultiSelected
							? () =>
									setMultiTableDraft((prev) =>
										prev.filter((_, i) => i !== multiDraftIdx),
									)
							: undefined,
					} as TableNodeData,
				};
			...
			...drawElements.map((item) => ({
				...
				data: {
					...
					onResize: (width, height) =>
						updateLiveDrawElementSize(item.id, width, height),
					onResizeEnd: (width, height) =>
						saveDrawElementSizeRef.current(item.id, width, height),
				} as DrawNodeData,
			})),
		]);
	}, [
		...
		multiTableMode,
		openEditModal,
		selectedDrawElementId,
		selectedTableId,
		seatedNamesByTable,
		setNodes,
		tableOccupancyMap,
		updateLiveDrawElementSize,
		updateLiveTableSize,
		visibleTables,
	]);
	...
	useEffect(() => {
		...
		ws.current = socket;
		socket.onopen = () => {
			for (const raw of pendingMapWSMessagesRef.current.splice(0))
				socket.send(raw);
		};
		...
		socket.onmessage = (event) => {
			try {
				...
				if (
					payload.type === "area_created" ||
					payload.type === "area_updated"
				) {
				...
				if (payload.type === "template_updated") {
					...
					const tpl = isNonEmptyTemplate(payload.template)
						? (payload.template as TableMapLayoutTemplate)
						: null;
					if (tpl) {
						setFloorTemplate(tpl);
				...
				if (payload.type === "layout_updated") {
					...
					const layout = (data.layout || payload.layout || {}) as Record<
						string,
						unknown
					>;
					const elements = Array.isArray(layout.elements)
						? ((layout.elements as any[])
								.map((item) => ({
									id: String(item?.id || ""),
									kind: normalizeDrawElementKind(item?.kind),
									preset: normalizeDrawElementPreset(item?.preset),
									displayMode: normalizeDrawElementDisplayMode(
										item?.display_mode ?? item?.displayMode,
									),
									x: Number(item?.x || 0),
									y: Number(item?.y || 0),
									width: Math.max(24, Number(item?.width || 24)),
									height: Math.max(24, Number(item?.height || 24)),
									rotationDeg: Number(item?.rotationDeg || 0),
				...
				if (
					...
					setAreas((prev) => {
						...
						const targetAreaID = Number(
							mergedTable.area_id || existingTable?.area_id || 0,
						);
	...
	const saveTableSize = useCallback(
		async (id: string, width: number, height: number) => {
			...
			const node = nodes.find(
				(item) => item.id === id && item.type === "restaurantTable",
			);
			if (node?.position) {
				const data = node.data as TableNodeData;
				const rotationDeg = Number.isFinite(data.rotationDeg)
					? data.rotationDeg
					: 0;
				const frame = rotatedRectFrameFromPosition(
					node.position,
	...
	const queuePersistLayout = useCallback(
		(
			...
			const history = getMapHistory();
			const after = cloneMapEditSnapshot({
				drawElements: elements,
				limitPoints,
			});
			if (!applyingHistoryRef.current) {
				if (!history.last) {
					...
					history.undo.push({
						before: cloneMapEditSnapshot(history.last),
						after: cloneMapEditSnapshot(after),
						timestamp: Date.now(),
					});
					history.redo = [];
					history.last = cloneMapEditSnapshot(after);
	...
	const applyMapEditSnapshot = useCallback(
		(snapshot: MapEditSnapshot) => {
	...
	const onNodesChange = useCallback(
		(changes: NodeChange[]) => {
			...
			const pendingTableSizeSaves: Array<{
				id: string;
				width: number;
				height: number;
			}> = [];
				...
				for (const c of changes as any[]) {
					if (c.type === "position") {
						...
						if (node.type === "restaurantTable") {
							...
							node.position = blockedByObstacle
								? prevNode.position
								: constrainedPosition;
							continue;
						}
					...
					if (c.type === "dimensions") {
						...
						if (node.type === "drawElement") {
							...
							const rotationDeg = Number.isFinite(prevData.rotationDeg)
								? prevData.rotationDeg
								: 0;
							...
							const nextFrame = rotatedRectFrameFromPosition(
								node.position,
								nextWidth,
								nextHeight,
								rotationDeg,
						...
						if (node.type === "restaurantTable") {
							...
							const fitsLimit = activeLimitPoints
								? isRectInsideLimitArea(candidateFrame, activeLimitPoints)
								: true;
							const fitsTables = tableSizeFitsOtherTables(
								node,
								candidateWidth,
								candidateHeight,
								next,
			...
			for (const c of changes as any[]) {
				if (c.type === "position" && c.dragging === false) {
					...
					if (
						String(c.id).startsWith("draw-") &&
						updatedNode.type === "drawElement"
					) {
				...
				if (c.type === "dimensions" && String(c.id).startsWith("draw-")) {
					...
					const updated = nextDrawElements.map((el) => {
						...
						const height = Math.max(
							24,
							Number(updatedData.height || el.height),
						);
	...
	const [flowViewport, setFlowViewport] = useState<FlowViewportTransform>({
		x: 0,
		y: 0,
		zoom: 1,
	});
	...
	const onDropBooking = useCallback(
		(event: React.DragEvent) => {
			...
			const currentTable = event.dataTransfer.getData(
				"application/booking-current-table",
			);
			if (!bookingId) return;
			...
			if (!targetNode) {
				pushToast({
					kind: "error",
					title: "Error",
					message: "Suelta la reserva sobre una mesa",
				});
				return;
			}
	...
	const onDragStart = useCallback(
		(event: React.DragEvent, booking: Booking) => {
		...
		try {
			...
			if (!res.success) {
				pushToast({
					kind: "error",
					title: "Error",
					message: (res as any).message || "No se pudo eliminar la mesa",
				});
				return;
			}
			setAreas((prev) =>
				prev.map((area) => ({
					...
					tables: (area.tables || []).filter(
						(t) => t.id !== deleteConfirmTableId,
					),
				})),
			);
			setSelectedTableId((prev) =>
				prev === deleteConfirmTableId ? null : prev,
			);
			...
			pushToast({
				...
				message: "La mesa se ha eliminado",
			});
		} catch (err) {
			pushToast({
				kind: "error",
	...
	const ensureAreaForFloor = useCallback(async (): Promise<number | null> => {
		...
		if (!createRes.success) {
			pushToast({
				kind: "error",
				title: "Error",
				message: createRes.message || "No se pudo crear area",
			});
			return null;
		}
		...
		try {
			...
			const payload: any = {
				...
				metadata: {
					...
					short_side_seats: shortSidesToMetadata(
						normalizeRectShortSides(draft.capacity, draft.rectShortSides),
					),
				},
			};
			if (draft.textureImageUrl)
				payload.texture_image_url = draft.textureImageUrl;
			...
			if (editingTableId) {
				...
				const updated =
					((res.table || res.item) as TableMapItem | undefined) || null;
				if (updated) {
					setAreas((prev) =>
						prev.map((area) => ({
							...area,
							tables: (area.tables || []).map((table) =>
								table.id === updated.id ? { ...table, ...updated } : table,
							),
						})),
					);
				}
			} else {
				const activeLimitPoints = hasClosedLimitArea(lineDrawing.points)
					? lineDrawing.points
					: null;
				if (!activeLimitPoints) {
					pushToast({
						kind: "error",
						title: "Limites requeridos",
						message: "Dibuja y cierra el area de limites primero.",
					});
					return;
				}

				const normalizedCapacity = clampCapacity(draft.capacity);
				const tableGeom = previewGeometry(
					draft.shape,
					normalizedCapacity,
					normalizeRectShortSides(draft.capacity, draft.rectShortSides),
				);
				const preferredPosition = {
					x: 140 + visibleTables.length * 24,
					y: 140 + visibleTables.length * 24,
				};
				...
				if (!boundedPosition) {
					pushToast({
						...
						title: "Sin espacio",
						message:
							"No hay espacio dentro del area limite para una nueva mesa.",
					});
					return;
				}
				...
				if (!res.success) {
					pushToast({
						kind: "error",
						title: "Error",
						message: res.message || "No se pudo crear mesa",
					});
					return;
				}
			...
			if (draftTextureFile && tableId) {
				const uploadRes = await api.tables.uploadTextureImage(
					tableId,
					draftTextureFile,
				);
				if (!uploadRes.success) {
					pushToast({
						kind: "error",
						title: "Imagen",
						message: uploadRes.message || "No se pudo subir imagen",
					});
				}
			}
			...
			pushToast({
				kind: "success",
				title: editingTableId ? "Mesa actualizada" : "Mesa creada",
			});
			setEditorOpen(false);
		} finally {
			setSaving(false);
	...
	const onTextureInput = useCallback(
		async (e: React.ChangeEvent<HTMLInputElement>) => {
			...
			try {
				...
				const webpFile = toFileFromDataURL(
					compressed,
					`${(draft.name || "table").replace(/\s+/g, "-")}.webp`,
				);
				...
				pushToast({
					kind: "error",
					title: "Imagen",
					message: "No se pudo procesar imagen",
				});
			}
		},
	...
	const onBack = useCallback(() => {
		...
		window.location.assign(
			`/app/reservas?date=${encodeURIComponent(selectedDate)}`,
		);
	}, [selectedDate]);
	...
	const onSelectDate = useCallback(
		(nextDate: string) => {
			...
			if (
				nextView.year !== calendarView.year ||
				nextView.month !== calendarView.month
			) {
			...
			if (typeof window !== "undefined") {
				window.history.replaceState(
					null,
					"",
					withDateParam(window.location.href, nextDate),
				);
			}
		},
	...
	const markBookingSeated = useCallback(
		(booking: Booking, seated: boolean) => {
			const prev = bookingStatesRef.current[String(booking.id)] || {
				seated: false,
			};
			const next = {
				...bookingStatesRef.current,
				[String(booking.id)]: { ...prev, seated },
			};
	...
	const saveBookingAssignments = useCallback(
		async (booking: Booking, assignments: BookingTableAssignment[]) => {
			const partySize = Math.max(
				1,
				Math.round(Number(booking.party_size) || 1),
			);
			const normalized = normalizeAssignmentSeats(
				assignments,
				partySize,
			) as BookingTableAssignment[];
			...
			if (normalized.some((a) => !normalizeTableKey(a.table_name))) {
				pushToast({
					kind: "error",
					title: "Mesas",
					message: "Selecciona una mesa para cada fila",
				});
				return;
			}
			...
			for (const a of normalized) {
				...
				if (seenTableNames.has(key)) {
					pushToast({
						kind: "error",
						title: "Mesas",
						message: `La mesa ${a.table_name} aparece repetida`,
					});
					return;
				}
			...
			const prevAssignments = resolveAssignments(
				prevState,
				booking.table_number,
				booking.party_size,
			);
			...
			for (const a of prevAssignments) {
				const key = normalizeTableKey(a.table_name);
				prevSeatsByTable.set(
					key,
					(prevSeatsByTable.get(key) || 0) + Math.max(0, Number(a.seats) || 0),
				);
			}
			for (const a of normalized) {
				const key = normalizeTableKey(a.table_name);
				const table = visibleTables.find(
					(t) => normalizeTableKey(t.name) === key,
				);
				if (!table) continue;
				const otherSeats = Math.max(
					0,
					(occupiedSeatsByTable.get(key) || 0) -
						(prevSeatsByTable.get(key) || 0),
				);
				if (
					otherSeats + Math.max(0, Number(a.seats) || 0) >
					(table.capacity || 4)
				) {
					pushToast({
						kind: "error",
			...
			setBookings((prev) =>
				prev.map((b) =>
					b.id === booking.id ? { ...b, table_number: primary || null } : b,
				),
			);
			...
			const res = await api.reservas.patch(booking.id, {
				table_number: primary,
			});
			if (!res.success) {
				setBookings((prev) =>
					prev.map((b) =>
						b.id === booking.id
							? { ...b, table_number: booking.table_number }
							: b,
					),
				);
				...
				pushToast({
					kind: "error",
					title: "Error",
					message: res.message || "No se pudo guardar la asignacion",
				});
				return;
			}
			if (normalized.length > 0) {
				pushToast({
					...
					message: `${booking.customer_name} -> ${display}`,
				});
			} else {
	...
	const addDrawElement = useCallback(
		(preset: DrawElementPreset) => {
			...
			const preferred = {
				x: 180 + current.length * 24,
				y: 180 + current.length * 24,
			};
			const base = findNearestRectInsideLimitArea(
				preferred,
				size,
				activeLimitPoints,
			);
			if (!base) {
				pushToast({
					kind: "error",
					title: "Sin espacio",
					message: "No hay espacio dentro del area limite para ese elemento.",
				});
				return;
			}
	...
	const selectedDrawElement = useMemo(
		() =>
			selectedDrawElementId
				? drawElements.find((item) => item.id === selectedDrawElementId) || null
				: null,
		[drawElements, selectedDrawElementId],
	);
	...
	const rotateSelectedDrawElement = useCallback(
		(direction: -1 | 1) => {
			...
			const updated = current.map((item) => {
				...
				const base =
					Math.round(
						(Number.isFinite(item.rotationDeg) ? item.rotationDeg : 0) /
							DRAW_ROTATE_STEP,
					) * DRAW_ROTATE_STEP;
				const next =
					(((base + direction * DRAW_ROTATE_STEP) % 360) + 360) % 360;
	...
	const reservasTabItems = useMemo<TabItem[]>(
		() => [
			...
			},
			{
				...
				label: "Mesas",
				href: "#mesas",
				icon: <LayoutGrid className="bo-ico" />,
			},
		],
		[],
	);
		...
		async (booking: Booking) => {
			...
			if (!res.success) {
				pushToast({
					kind: "error",
					title: "Error",
					message: res.message || "No se pudo cancelar",
				});
				return;
			}
	...
	const multiTableTotalSeats = useMemo(
		() => sumAssignmentSeats(multiTableDraft),
		[multiTableDraft],
	);
	const multiTablePartySize = bookingForAssignment
		? Math.max(1, Math.round(Number(bookingForAssignment.party_size) || 1))
		: 0;
	...
	const addTableToMultiDraft = useCallback((table: TableMapItem) => {
		...
		setMultiTableDraft((prev) => {
			...
			if (prev.some((a) => normalizeTableKey(a.table_name) === key))
				return prev;
			return [
				...prev,
				{
	...
	const updateMultiDraftNames = useCallback((idx: number, names: string[]) => {
		setMultiTableDraft((prev) =>
			prev.map((row, i) => (i === idx ? { ...row, names } : row)),
		);
	}, []);
	...
	const saveMultiTableDraft = useCallback(async () => {
		...
		await saveBookingAssignmentsRef.current(
		...
		pushToast({
			...
			message: `${multiTableDraft.length} mesa(s) asignadas`,
		});
	...
	const handleAssignModeSelect = useCallback(
		async (bookingId: number, tableId: number) => {
			...
			const already = existing.some(
				(a) =>
					normalizeTableKey(a.table_name) === normalizeTableKey(table.name),
			);
			const assignments: BookingTableAssignment[] = already
				? existing
				: [
	...
	const placeMenuTooltip = useCallback(() => {
		...
		const left = Math.max(
			M,
			Math.min(centerX - tt.width / 2, vw - tt.width - M),
		);
		const belowTop = br.bottom + 8;
		const top =
			belowTop + tt.height + M > vh
	...
	useEffect(() => {
		...
		const onClickOutside = (e: MouseEvent) => {
			...
			const insidePortalUI = Boolean(
				target.closest?.(
					'[data-ui="modal-overlay"], [data-ui="dropdown-menu"]',
				),
			);
			if (menuVisible) {
	...
	const closeLineDrawing = useCallback(() => {
		if (lineDrawing.points.length < 3) {
			pushToast({
				kind: "error",
				title: "Area invalida",
				message: "Necesitas al menos 3 puntos para cerrar el area.",
			});
			return;
		}
	...
	const startLimitAreaEditing = useCallback(() => {
		if (!hasClosedLimitArea(lineDrawing.points) || lineDrawing.isDrawing)
			return;
		limitEditHistoryRef.current = [];
	...
	const saveLimitAreaTemplate = useCallback(async () => {
		if (lineDrawing.isDrawing || !hasClosedLimitArea(lineDrawing.points)) {
			pushToast({
				kind: "error",
				title: "Area invalida",
				message: "Cierra el area antes de guardar la plantilla.",
			});
			return;
		}
		...
		try {
			...
			if (templateScope === "template" || !floorTemplate) {
				...
				const newLayout = stripTemplateFieldsForDay(
					buildGlobalTemplateLayout({
						...stripDayFieldsForTemplate({ ...payload }),
					}),
				);
				queuePersistLayout(
					clonedElements,
			...
			const dayOverride = buildDayOverrideLayout(
				{
					limit_area_template_points: clonedPoints,
					draw_elements_template: clonedElements,
				},
				{},
			);
			sendMapWSMessage({
	...
	const applyScopeChange = useCallback(
		async (next: TableMapTemplateScope) => {
			...
			if (next === "day") {
				...
				for (const table of visibleTables) {
					tablePositions[String(table.id)] = {
						x_pos: Number(table.x_pos || 0),
						y_pos: Number(table.y_pos || 0),
					};
				}
				const dayOverride = buildDayOverrideLayout(
					floorTemplate,
	...
	const onVertexDragStart = useCallback((_index: number) => {
		limitEditHistoryRef.current.push(
			cloneLinePoints(lineDrawingPointsRef.current),
		);
	}, []);

	const onVertexDragMove = useCallback(
		(index: number, flowPoint: LinePoint) => {
			setLineDrawing((prev) => {
				if (!prev.points[index]) return prev;
				const nextPoints = prev.points.map((point, idx) =>
					idx === index ? flowPoint : point,
				);
				lineDrawingPointsRef.current = nextPoints;
		...
		(index: number) => {
			...
			if (current.length <= 3) {
				pushToast({
					kind: "info",
					title: "Area invalida",
					message: "Necesitas al menos 3 puntos en el area.",
				});
				return;
			}
	...
	const removeArea = useCallback(() => {
		...
		pushToast({
			...
			message: "Limites y elementos del mapa eliminados.",
		});
	...
	const lineOverlayPoints = useMemo(
		() =>
			lineDrawing.points.map((point) =>
				projectFlowPointToOverlay(point, flowViewport),
			),
	...
	return (
		<ReactFlowProvider>
			<section
				data-ui="table-map-page"
				className="bo-tableMapPage"
				aria-label="Mapa de mesas"
			>
				{loading ? (
					<div data-ui="loading" className="bo-tableMapLoading">
						Cargando mapa...
					</div>
				) : null}
				<AnimatePresence mode="wait" initial={false}>
					{isDayOpen ? (
						...
						>
							<div data-ui="top-controls" className="bo-tableMapTopControls">
								...
								<div data-ui="top-center" className="bo-tableMapTopCenter">
									...
									<AnimatePresence>
										{menuVisible ? (
											...
											>
												<div
													data-slot="tooltip-head"
													className="bo-tableMapTooltipHead"
												>
													<div
														data-ui="tooltip-title"
														className="bo-tableMapTooltipTitle"
													>
														Mapa de mesas
													</div>
													<div
														data-ui="tooltip-subtitle"
														className="bo-tableMapTooltipSub"
													>
														Acciones rapidas
													</div>
												</div>
												...
												<div
													...
													className="bo-tableMapTooltipActions"
													role="group"
													aria-label="Acciones de mapa"
												>
													<button
														data-ui="add-table-btn"
													...
													>
														...
														<span data-ui="menu-label" className="bo-menuLabel">
															{editMode ? "Salir de dibujo" : "Dibujar"}
														</span>
													</button>
												</div>
												...
												>
													<div data-ui="stat-people">
														Personas / Limite:{" "}
														<strong data-ui="people-value">
															{occupancy.totalPeople} / {occupancy.limit || "-"}
														</strong>
													</div>
													<div data-ui="stat-occupancy">
														Ocupacion:{" "}
														<strong data-ui="occupancy-value">
															{occupancy.percent}%
														</strong>
													</div>
												</div>
												...
												{floorTabs.length > 1 ? (
													<div
														data-ui="floor-tabs"
														className="bo-tableMapFloorTabs"
														role="tablist"
								...
								<div data-ui="top-right" className="bo-tableMapTopRight">
									<div
										...
										className={`bo-tableMapEditToggleWrap${editMode ? " is-active" : ""}`}
									>
										<span
										...
										<Switch
											...
											title={
												editMode
													? "Modo edicion (desactivar para solo lectura)"
													: "Modo lectura (activar para editar)"
											}
								...
								<ReactFlow
									...
									onNodeClick={(_event, node) => {
										if (node.type === "restaurantTable") {
											...
											if (assignMode) {
												setSelectedTableId((prev) =>
													prev === tableData.id ? null : tableData.id,
												);
												...
												const tableBookings = bookings.filter((b) => {
													const assignments = resolveAssignments(
														bookingStates[String(b.id)],
														b.table_number,
														b.party_size,
													);
													return assignments.some(
														(a) => normalizeTableKey(a.table_name) === tableKey,
													);
												});
								...
								>
									...
									{controlsCollapsed ? (
										<Panel
											...
											className="bo-tableMapControlsExpand"
											data-ui="controls-expand-panel"
										>
											<button
										...
										<Controls>
											...
											<ControlButton
												...
												className={
													interactionMode === "select" ? "is-active" : ""
												}
							...
							>
								<div
									data-slot="draw-panel-head"
									className="bo-tableMapDrawPanelHead"
								>
									<div data-ui="draw-panel-title" className="bo-panelTitle">
										Dibujo
									</div>
									<button
										data-ui="close-draw-panel-btn"
										className="bo-btn bo-btn--ghost"
										type="button"
										onClick={closeDrawPanel}
									>
										Cerrar
									</button>
								</div>
								<div
									data-slot="draw-panel-body"
									className="bo-tableMapDrawPanelBody"
								>
									<ScrollArea
									...
									>
										...
										{floorTemplate ? (
											<div
												...
												className="bo-tableMapDrawSection bo-tableMapScopeSection"
											>
												<div
													data-ui="scope-title"
												...
												{templateScope === "day" ? (
													<div
														data-ui="day-scope-hint"
														className="bo-tableMapDrawHint bo-tableMapDrawHint--compact"
													>
														Estás editando solo para esta fecha. La plantilla no
														cambiara.
													</div>
												) : (
													<div
														data-ui="template-scope-hint"
														className="bo-tableMapDrawHint bo-tableMapDrawHint--compact"
													>
														Los cambios se aplican a todos los dias de este
														salon.
													</div>
												)}
											</div>
										...
										<div
											data-ui="limit-section"
											className="bo-tableMapDrawSection"
										>
											<div
												data-ui="limit-header"
											...
											currentMapHistory.undo.length === 0 ? (
												<button
													data-ui="start-line-drawing-btn"
												...
												<div
													data-ui="line-draw-controls"
													className="bo-tableMapLineDrawControls"
												>
													{lineDrawing.isDrawing && (
														<div
															data-ui="line-draw-status-row"
															className="bo-tableMapLineDrawStatusRow"
														>
															<div
																data-ui="line-draw-status"
																className="bo-tableMapLineDrawStatus"
															>
																<Circle
																	size={12}
																	className="bo-tableMapLineDrawStatusDot"
																/>
																<span data-ui="point-count">
																	{lineDrawing.points.length} puntos
																</span>
															</div>
															<span
																data-ui="line-draw-status-hint"
																className="bo-tableMapLineDrawStatusHint"
															>
																Pulsa en el mapa para añadir vértices
															</span>
														</div>
													)}
													<div
														data-ui="line-draw-primary-actions"
														className="bo-tableMapLineDrawActionGroup bo-tableMapLineDrawActionGroup--primary"
													>
														{lineDrawing.isDrawing &&
															lineDrawing.points.length > 0 && (
																<button
																	data-ui="undo-point-btn"
																	className="bo-btn bo-btn--ghost bo-btn--sm"
																	type="button"
																	onClick={undoCreateAreaLastAction}
																>
																	<Undo size={14} />
														...
														{lineDrawing.points.length >= 3 &&
															lineDrawing.isDrawing && (
																<button
																	data-ui="close-area-btn"
																	className="bo-btn bo-btn--primary bo-btn--sm"
																	type="button"
																	onClick={closeLineDrawing}
																>
																	<SquareMinus size={14} />
																	Cerrar area
																</button>
														...
														{!lineDrawing.isDrawing &&
															...
															!isEditingLimitArea && (
																<button
																	data-ui="edit-area-btn"
																	className="bo-btn bo-btn--primary bo-btn--sm"
																	type="button"
																	onClick={startLimitAreaEditing}
																>
																	<Pencil size={14} />
																	Editar area
																</button>
														...
														{!lineDrawing.isDrawing &&
															...
															isEditingLimitArea && (
																<button
																	data-ui="save-edit-btn"
																	className="bo-btn bo-btn--primary bo-btn--sm"
																	type="button"
																	onClick={stopLimitAreaEditing}
																>
																	<SquareMinus size={14} />
																	Guardar edicion
																</button>
													...
													{isEditingLimitArea && (
														<div
															data-ui="line-draw-edit-actions"
															className="bo-tableMapLineDrawActionGroup bo-tableMapLineDrawActionGroup--secondary"
														>
															<span
																data-ui="editing-badge"
																className="bo-tableMapLineDrawEditingBadge"
															>
																Editando límites
															</span>
															<button
																data-ui="undo-edit-btn"
																className="bo-btn bo-btn--ghost bo-btn--sm"
																type="button"
																onClick={undoEditAreaLastAction}
															>
																<Undo size={14} />
													...
													<div
														data-ui="line-draw-persistence-actions"
														className="bo-tableMapLineDrawActionGroup bo-tableMapLineDrawActionGroup--persistence"
													>
														{currentMapHistory.undo.length > 0 && (
															<button
										...
										>
											...
											>
												<div
													...
													className="bo-drawAccordionToggleLabel"
												>
													<div
														data-ui="draw-section-title"
														className="bo-tableMapDrawSectionTitle"
													>
														Elementos
													</div>
													<div
														data-ui="draw-section-hint"
														className="bo-tableMapDrawHint"
													>
														Anade objetos con un solo click.
													</div>
												</div>
												<ChevronDown
													size={16}
													className={`bo-drawAccordionChevron${elementosOpen ? " is-open" : ""}`}
												/>
											</button>
											{elementosOpen && (
												<div
													data-ui="draw-preset-groups"
													className="bo-drawPresetGroups"
													aria-label="Herramientas de dibujo"
												>
													{DRAW_PANEL_GROUPS.map((group) => (
														<section
															key={group.id}
															data-ui="draw-preset-group"
															className="bo-drawPresetGroup"
															aria-label={group.title}
														>
															<div
																data-ui="draw-group-title"
																className="bo-drawPresetGroupTitle"
															>
																{group.title}
															</div>
															<div
																data-ui="draw-preset-grid"
																className="bo-drawPresetGrid"
															>
																{group.presets.map((preset) => {
																	const previewUrl =
																		drawPresetAssetImageUrl(preset);
																	const isActivePreset =
																		selectedDrawElement?.preset === preset;
																	return (
																		<button
																			key={preset}
																		...
																		>
																			<span
																				data-ui="preset-icon"
																				className="bo-drawPresetBtnIcon"
																				aria-hidden="true"
																			>
																				{previewUrl ? (
																					<img
																						data-ui="preset-asset"
																						className="bo-drawPresetBtnAsset"
																						src={previewUrl}
																						alt=""
																					/>
																				) : (
																			...
																			<span
																				data-ui="preset-label"
																				className="bo-drawPresetBtnLabel"
																			>
																				{drawPresetLabel(preset)}
																			</span>
																		</button>
																	);
																})}
										...
										<div
											data-ui="draw-visual-section"
											className="bo-tableMapDrawSection"
										>
											<div
												data-ui="draw-visual-title"
												className="bo-tableMapDrawSectionTitle"
											>
												Visual del elemento
											</div>
											{selectedDrawElement ? (
												<>
													<div
														data-ui="selected-hint"
														className="bo-tableMapDrawHint bo-tableMapDrawHint--compact"
													>
														Seleccionado:{" "}
														<strong data-ui="selected-element-name">
															{selectedDrawElement.label}
														</strong>
													</div>
													<div
														data-ui="rotation-controls"
														className="bo-drawRotationControls"
														role="group"
														aria-label="Rotacion del elemento"
													>
														<button
															data-ui="rotate-left-btn"
															className="bo-drawRotateBtn"
															type="button"
															onClick={() => rotateSelectedDrawElement(-1)}
														>
															<RotateCcw size={14} />
															-10°
														</button>
														<div
															data-ui="rotation-value"
															className="bo-drawRotationValue"
														>
															{Math.round(selectedDrawElement.rotationDeg)}°
														</div>
														<button
													...
													<div
														data-ui="display-mode-picker"
														className="bo-drawDisplayModePicker"
														role="group"
														aria-label="Modo de visualizacion del elemento"
													>
														<button
															...
															type="button"
															onClick={() =>
																updateSelectedDrawElementDisplayMode("both")
															}
														>
															Ambos
														</button>
														<button
															...
															type="button"
															onClick={() =>
																updateSelectedDrawElementDisplayMode("asset")
															}
														>
															Solo asset
														</button>
														<button
															...
															type="button"
															onClick={() =>
																updateSelectedDrawElementDisplayMode("text")
															}
														>
															Solo texto
														</button>
												...
												<div
													data-ui="no-element-hint"
													className="bo-tableMapDrawHint"
												>
													Selecciona un elemento del mapa para cambiar su
													visual.
												</div>
											)}
										</div>
							...
							<aside
								ref={rightSheetRef as React.RefObject<HTMLElement>}
								data-ui="right-sheet"
								className={`bo-tableMapSheet${rightSheetOpen ? " is-open" : ""}${isDragging ? " drag-active" : ""}`}
								aria-label="Panel de reservas"
							>
								<div data-slot="sheet-head" className="bo-tableMapSheetHead">
									{bookingForAssignment ? (
										...
										<div
											data-ui="sheet-stats"
											className="bo-tableMapSheetStats"
										>
											<span
												data-ui="stat-total"
												className="bo-tableMapSheetStat bo-tableMapSheetStat--total"
											>
												<span
													data-ui="stat-dot"
													className="bo-tableMapSheetStatDot"
												/>
												{bookingStats.total} reservas
											</span>
											<span
												data-ui="stat-seated"
												className="bo-tableMapSheetStat bo-tableMapSheetStat--seated"
											>
												<span
													data-ui="stat-dot"
													className="bo-tableMapSheetStatDot"
												/>
												{bookingStats.seated} sentadas
											</span>
											<span
												data-ui="stat-pending"
												className="bo-tableMapSheetStat bo-tableMapSheetStat--pending"
											>
												<span
													data-ui="stat-dot"
													className="bo-tableMapSheetStatDot"
												/>
												{bookingStats.pending} pendientes
											</span>
										</div>
									)}
									<div
										data-slot="sheet-header"
										className="bo-tableMapSheetHeader"
									>
										<div
											data-slot="sheet-header-left"
											className="bo-tableMapSheetHeaderLeft"
										>
											<div data-ui="sheet-title" className="bo-panelTitle">
												Booking manager
											</div>
											<div data-ui="sheet-meta" className="bo-panelMeta">
												{visibleTables.length} mesas
											</div>
										</div>
										<div
											data-slot="sheet-header-actions"
											className="bo-tableMapSheetHeaderActions"
										>
											<button
												data-ui="date-toggle-btn"
												className="bo-btn bo-btn--ghost bo-tableMapDateBtn"
												type="button"
								...
								<div data-slot="sheet-body" className="bo-tableMapSheetBody">
									{calendarExpanded ? (
										<div
											data-ui="calendar-wrapper"
											className="bo-tableMapCalendarWrapper"
										>
											<MonthCalendar
												year={calendarView.year}
												month={calendarView.month}
									...
									{floorTabs.length > 1 && (
										<div
											data-ui="sheet-floor-tabs"
											className="bo-tableMapFloorTabs"
											role="tablist"
											aria-label="Seleccionar salon/planta"
										>
											{floorTabs.map((floor) => {
									...
									<div
										...
										className="bo-tableMapSheetContent"
									>
										<ScrollArea dataSlot="sheet-content">
											{sheetTab === "reservas" ? (
												<div
													data-ui="reservations-section"
													className="bo-tableMapSection"
												>
													<div
														data-slot="reservations-header"
														className="bo-tableMapSectionHeader"
													>
														<div
															data-ui="reservations-title"
															className="bo-tableMapSectionTitle"
														>
															Reservas del dia
														</div>
														{bookings.length > 0 &&
													...
													{bookings.length === 0 ? (
														<div
															data-ui="empty-bookings"
															className="bo-tableMapEmptyState"
														>
															<div
																data-ui="empty-icon"
																className="bo-tableMapEmptyStateIcon"
															>
																<CalendarDays size={24} />
															</div>
															<div data-ui="empty-text">
														...
														<div
															data-ui="bookings-list"
															className="bo-tableMapBookingsList"
														>
															{assignMode && hasUnassignedBookings && (
																<div
																	data-ui="assign-hint"
																	className="bo-tableMapAssignModeHint"
																>
																	Selecciona una reserva sin mesa asignada
																</div>
															)}
															{assignMode && !hasUnassignedBookings && (
																<div
																	data-ui="all-assigned"
																	className="bo-tableMapEmptyState"
																>
																	<div
																		data-ui="empty-icon"
															...
															{(
																bookings.filter(
																	(b) => !assignMode || !b.table_number,
																) || []
															).map((booking) => {
																const seated =
																	bookingStates[String(booking.id)]?.seated;
																const isUnassigned = !booking.table_number;
																const isAssigning =
																	bookingForAssignment?.id === booking.id;
																const isSelected =
																...
																return (
																	<React.Fragment key={booking.id}>
																		...
																		{assignMode && isSelected && (
																			<div
																				data-ui="multi-table-inline"
																				className="bo-multiTableInline"
																			>
																				<div
																				...
																				{multiTableMode && (
																					<>
																						<div
																							data-ui="multi-table-hint"
																							className="bo-multiTableHint"
																						>
																							Haz clic en las mesas del mapa
																							para añadirlas
																						</div>
																						...
																						{multiTableDraft.length > 0 && (
																							<div
																								data-ui="multi-table-assigned"
																								className="bo-multiTableAssigned"
																							>
																								{multiTableDraft.map(
																									(row, idx) => (
																										<div
																						...
																						{multiTableDraft.length > 0 && (
																							<div
																								data-ui="multi-table-actions"
																								className="bo-multiTableActions"
																							>
																								<button
																			...
																			onClick={() => {
																				...
																				if (assignMode) {
																					setSelectedBookingId(
																						isSelected ? null : booking.id,
																					);
																				} else if (
																					bookingForAssignment?.id ===
																					booking.id
																				) {
																					setBookingForAssignment(null);
																				} else {
																		...
																		>
																			{assignMode ? (
																				<label
																					data-ui="booking-checkbox"
																					className="bo-checkboxContainer"
																				...
																				>
																					...
																					<span
																						...
																						className="bo-checkboxMark"
																					/>
																				</label>
																			) : (
																				<span
																					data-ui="drag-handle"
																					className="bo-bookingDragIndicator"
																				>
																					<GripVertical size={16} />
																				</span>
																			)}
																			<span
																				data-ui="booking-status-dot"
																				className="bo-tableMapBookingStatusDot"
																			/>
																			<div
																				data-ui="booking-main"
																				className="bo-tableMapBookingMain"
																			>
																				...
																				<span data-ui="booking-pax-time">
																					{booking.party_size} pax ·{" "}
																					{formatHHMM(booking.reservation_time)}
																				</span>
																			</div>
																			<DropdownMenu
													...
													selectedTableCard ? (
														...
														>
															<div
																...
																className="bo-tableSheetDetailHeader"
															>
																<button
																	data-ui="back-to-list-btn"
																	className="bo-actionBtn bo-actionBtn--glass"
																	type="button"
																...
																<div
																	data-ui="detail-table-info"
																	className="bo-tableSheetDetailTableInfo"
																>
																	<span
																		data-ui="detail-table-name"
																		className="bo-tableSheetDetailTableName"
																	>
																		{selectedTableCard.name}
																	</span>
																	<span
																		data-ui="detail-table-cap"
																		className="bo-tableSheetDetailTableCap"
																	>
																		{selectedTableCard.capacity} pax
																	</span>
																</div>
																<span
																	data-ui="detail-status-pill"
																	className={`bo-tableSheetDetailStatusPill${selectedTableCardIsOccupied ? " is-occupied" : " is-free"}`}
																>
															...
															{selectedTableCardBookings.length > 0 ? (
																<div
																	data-slot="detail-bookings"
																	className="bo-tableSheetDetailBookings"
																>
																	{selectedTableCardBookings.map((booking) => {
																		...
																		return (
																			<div
																				key={booking.id}
																				data-ui="detail-booking-card"
																				className="bo-tableSheetDetailBookingCard"
																			>
																				<div
																					data-slot="booking-card-head"
																					className="bo-tableSheetDetailBookingHead"
																				>
																					<div
																						data-ui="booking-customer"
																						className="bo-tableSheetDetailBookingCustomer"
																					>
																						{booking.customer_name}
																					</div>
																					<span
																						data-ui="booking-status-pill"
																						className={`bo-tableSheetDetailBookingStatus${isSeated ? " is-seated" : " is-pending"}`}
																					>
																						{isSeated ? "Sentada" : "Pendiente"}
																					</span>
																				</div>
																				<div
																					data-slot="booking-card-meta"
																					className="bo-tableSheetDetailBookingMeta"
																				>
																					<span data-ui="booking-pax">
																						{booking.party_size} pax
																					</span>
																					<span data-ui="booking-time">
																						{formatHHMM(
																							booking.reservation_time,
																						)}
																					</span>
																					{booking.contact_phone && (
																						<span data-ui="booking-phone">
																							{booking.contact_phone}
																						</span>
																					)}
																				</div>
																				{booking.commentary ? (
																					<div
																						data-ui="booking-comment"
																						className="bo-tableSheetDetailBookingComment"
																					>
																						{booking.commentary}
																					</div>
																				) : null}
																				<div
																					data-slot="booking-card-actions"
																					className="bo-tableSheetDetailBookingActions"
																				>
																					<button
														...
														>
															<div
																data-slot="tables-header"
																className="bo-tableMapSectionHeader"
															>
																<div
																	data-ui="tables-title"
																	className="bo-tableMapSectionTitle"
																>
																	Estado de mesas
																</div>
																<div
																	data-ui="tables-summary"
																	className="bo-tableMapSectionSummary"
																>
																	<span
																		data-ui="summary-free"
																		className="bo-tableMapSummaryItem bo-tableMapSummaryItem--free"
																	>
																		{tableSummary.free} libres
																	</span>
																	<span
																		data-ui="summary-booked"
																		className="bo-tableMapSummaryItem bo-tableMapSummaryItem--booked"
																	>
																		{tableSummary.booked} reservadas
																	</span>
																	<span
																		data-ui="summary-seated"
																		className="bo-tableMapSummaryItem bo-tableMapSummaryItem--seated"
																	>
																		{tableSummary.seated} ocupadas
																	</span>
																</div>
															</div>
															{visibleTables.length === 0 ? (
																<div
																	data-ui="empty-tables"
																	className="bo-tableMapEmptyState"
																>
																	<div
																		data-ui="empty-icon"
																	...
																	<div data-ui="empty-text">
																		No hay mesas en este salon
																	</div>
																	<button
																		data-ui="create-table-btn"
																		className="bo-btn bo-btn--ghost bo-btn--sm"
																		type="button"
																...
																<div
																	data-ui="tables-by-status"
																	className="bo-tableMapTablesByStatus"
																>
																	{tablesByStatus.seated.length > 0 && (
																		<div
																			data-ui="status-group-seated"
																			className="bo-tableMapTablesStatusGroup"
																		>
																			<div
																				data-ui="status-group-title"
																				className="bo-tableMapTablesStatusGroupTitle"
																			>
																				Ocupadas
																			</div>
																			<div
																				data-ui="status-group-grid"
																				className="bo-tableMapTablesGrid"
																			>
																				{tablesByStatus.seated.map((table) => {
																					const tableBookings =
																						getTableBookings(table.name);
																					...
																					return (
																						...
																						>
																							<span
																								data-ui="table-card-occ"
																								className="bo-tableMapTableCardOcc"
																							/>
																							<span
																								data-ui="table-card-name"
																								className="bo-tableMapTableCardNum"
																							>
																								{table.name}
																							</span>
																							<span
																								data-ui="table-card-cap"
																								className="bo-tableMapTableCardCap"
																							>
																								{table.capacity} pax
																							</span>
																							{currentBooking && (
																								<span
																									data-ui="table-card-booking"
																									className="bo-tableMapTableCardBooking"
																								>
																									{
																										currentBooking.customer_name?.split(
																											" ",
																										)[0]
																									}{" "}
																									·{" "}
																									{formatHHMM(
																										currentBooking.reservation_time,
																									)}
																								</span>
																							)}
																						</div>
																		...
																		>
																			...
																			<AnimatePresence>
																				...
																				!selectedTableCardIsOccupied ? (
																					...
																					>
																						<div
																							data-ui="assign-title"
																							className="bo-tableSheetAssignTitle"
																						>
																							Asignar reserva a{" "}
																							<strong>
																								{selectedTableCard.name}
																						...
																						{unassignedBookings.length > 0 ? (
																							<div
																								data-ui="assign-list"
																								className="bo-tableSheetAssignList"
																							>
																								{unassignedBookings.map(
																									(booking) => (
																										...
																										>
																											<span
																												data-ui="assign-row-name"
																												className="bo-tableSheetAssignRowName"
																											>
																												{booking.customer_name}
																											</span>
																											<span
																												data-ui="assign-row-meta"
																												className="bo-tableSheetAssignRowMeta"
																											>
																												{booking.party_size} pax
																												·{" "}
																												{formatHHMM(
																							...
																							<div
																								data-ui="no-unassigned-bookings"
																								className="bo-tableSheetAssignEmpty"
																							>
																								No hay mas reservas para el dia
																								de hoy
																							</div>
																						)}
																					</motion.div>
							...
							<Modal
								open={editorOpen}
								title={editingTableId ? "Editar mesa" : "Nueva mesa"}
								onClose={() => setEditorOpen(false)}
								widthPx={980}
								className="bo-tableEditorModal"
								hideClose
							...
							>
								...
								<div data-ui="editor-grid" className="bo-tableEditorGrid">
									<ScrollArea
										dataSlot="editor-grid"
										className="bo-tableEditorGridScroll"
									>
										<div
											data-slot="editor-columns"
											className="bo-tableEditorColumns"
										>
											<div
												data-slot="editor-preview"
												className="bo-tableEditorPreviewWrap"
											>
												<div
													data-ui="rotate-controls"
													className="bo-tableEditorRotate"
													role="group"
													aria-label="Giro de mesa"
												>
													<button
														data-ui="rotate-left-btn"
														type="button"
														...
														onClick={() =>
															setDraft((prev) => {
																const base =
																	Math.round(
																		prev.rotationDeg / DRAW_ROTATE_STEP,
																	) * DRAW_ROTATE_STEP;
																return {
																	...prev,
																	rotationDeg: Math.max(
																		-180,
																		base - DRAW_ROTATE_STEP,
																	),
																};
															})
														}
												...
												<div
													data-ui="preview-table"
													className={`bo-tableEditorPreviewTable is-${draft.shape}`}
													style={{
														["--bo-table-fill" as any]: draft.fillColor,
														["--bo-table-outline" as any]: draft.outlineColor,
														["--bo-table-texture" as any]: draft.texturePreview
															? `url(${draft.texturePreview})`
															: "none",
														width: `${geom.width}px`,
														height: `${geom.height}px`,
												...
												>
													<span
														data-ui="preview-capacity"
														className="bo-tableEditorCapacity"
													>
														{clampCapacity(draft.capacity)}
													</span>
													{isRectangularPreview
														? (
																[
																	{
																		side: "left" as const,
																		canAdd: canAddLeftShortSide,
																		x: -(geom.width / 2 + RECT_SEAT_OFFSET),
																	},
																	{
																		side: "right" as const,
																		canAdd: canAddRightShortSide,
																		x: geom.width / 2 + RECT_SEAT_OFFSET,
																	},
																] as const
															).map((slot) => {
																...
																return (
																	<button
																		...
																		style={{
																			transform: `translate(${slot.x}px, 0px)`,
																		}}
																		onMouseEnter={() => armShortSide(slot.side)}
																		onMouseLeave={() =>
																			disarmShortSide(slot.side)
																		}
													...
													{geom.chairs.map((chair, idx) => {
														const shortSide: RectShortSide | null =
															chair.side === "left" || chair.side === "right"
																? chair.side
																: null;
														if (isRectangularPreview && shortSide) {
															return (
																...
																>
																	<span
																		data-ui="chair-action-icon"
																		className="bo-tableEditorChairAction"
																		aria-hidden="true"
																	>
											...
											<div
												data-slot="editor-config"
												className="bo-tableEditorConfig"
											>
												...
												<div data-ui="field-shape" className="bo-field">
													<label data-ui="shape-label" className="bo-label">
														Forma
													</label>
													<div
														data-ui="shape-buttons"
														className="bo-tableEditorShapeBtns"
													>
														<button
															data-ui="shape-round-btn"
															type="button"
															className={`bo-btn bo-btn--ghost${draft.shape === "round" ? " is-active" : ""}`}
															onClick={() =>
																setDraft((prev) => ({
																	...prev,
																	shape: "round",
														...
														<button
															data-ui="shape-square-btn"
															type="button"
															className={`bo-btn bo-btn--ghost${draft.shape === "square" ? " is-active" : ""}`}
															onClick={() =>
																setDraft((prev) => ({
																	...prev,
																	shape: "square",
												...
												<div data-ui="field-colors" className="bo-field">
													<label data-ui="colors-label" className="bo-label">
														Colores
													</label>
													<div
														data-ui="color-presets"
														className="bo-tableEditorPresetGrid"
													>
														{COLOR_PRESETS.map((preset) => (
															<button
																...
																style={{
																	["--bo-preset-fill" as any]: preset.fill,
																	["--bo-preset-outline" as any]:
																		preset.outline,
																}}
															/>
														))}
												...
												<div data-ui="field-texture" className="bo-field">
													<label data-ui="texture-label" className="bo-label">
														Subir textura
													</label>
													<label
														data-ui="texture-upload-btn"
														className="bo-btn bo-btn--ghost bo-tableUploadBtn"
													>
														<ImagePlus size={16} strokeWidth={1.8} />
														<span data-ui="upload-label">Subir imagen</span>
														<input
															data-ui="texture-file-input"
															type="file"
															accept="image/jpeg,image/png,image/webp,image/gif"
															onChange={onTextureInput}
															hidden
														/>
													</label>
												</div>
												...
												<PlusMinusCounter
													...
													onDecrease={() =>
														setDraftCapacity(draft.capacity - 1)
													}
													onIncrease={() =>
														setDraftCapacity(draft.capacity + 1)
													}
								...
								<div
									data-ui="editor-modal-actions-wrapper"
									className="bo-tableEditorActionsWrap"
								>
									<div
										data-ui="editor-modal-actions"
										className="bo-modalActions"
									>
										<button
											data-ui="cancel-editor-btn"
											className="bo-btn bo-btn--ghost"
											type="button"
							...
							<Modal
								...
								widthPx={760}
								className="bo-tableBookingModal"
							>
								{selectedBooking ? (
									<div
										data-ui="booking-modal-content"
										className="bo-tableBookingModalContent"
									>
										<div
											data-slot="modal-head"
											className="bo-tableBookingModalHead"
										>
											<button
												data-ui="close-booking-x"
												className="bo-modalX"
												type="button"
										...
										<div data-ui="booking-hero" className="bo-tableBookingHero">
											<div
												data-ui="booking-hero-left"
												className="bo-tableBookingHeroLeft"
											>
												{(() => {
													const display = assignmentsDisplayName(
														resolveAssignments(
															bookingStates[String(selectedBooking.id)],
													...
													return (
														<div
															data-ui="booking-table-badge"
															className={`bo-tableBookingTableBadge${unassigned ? " is-unassigned" : ""}`}
														>
												...
												<div
													data-ui="booking-hero-name"
													className="bo-tableBookingHeroName"
												>
													{selectedBooking.customer_name}
												</div>
											</div>
											<div
												data-ui="booking-hero-right"
												className="bo-tableBookingHeroRight"
											>
												<span
													data-ui="hero-pax"
													className="bo-tableBookingHeroStat"
												>
												...
												<span
													data-ui="hero-status"
													className={`bo-tableBookingHeroStatus${bookingStates[String(selectedBooking.id)]?.seated ? " is-seated" : " is-pending"}`}
												>
													{bookingStates[String(selectedBooking.id)]?.seated
														? "Sentada"
														: "Pendiente"}
												</span>
											</div>
										...
										<div
											data-ui="booking-details-grid"
											className="bo-tableBookingDetailsGrid"
										>
											<div
												data-ui="detail-name"
												className="bo-tableBookingDetailCard"
											>
												<div
													data-ui="detail-label"
													className="bo-tableBookingDetailLabel"
												>
													Nombre
												</div>
												<div
													data-ui="detail-value"
													className="bo-tableBookingDetailValue"
												>
													{selectedBooking.customer_name}
												</div>
											</div>
											...
											>
												...
												>
													<span
														...
														className={`bo-tableBookingStatusIndicator${bookingStates[String(selectedBooking.id)]?.seated ? " is-seated" : " is-pending"}`}
													/>
													{bookingStates[String(selectedBooking.id)]?.seated
														? "Sentada"
														: "Pendiente"}
											...
											{selectedBooking.commentary ? (
												<div
													data-ui="detail-comment"
													className="bo-tableBookingDetailCard bo-tableBookingDetailCard--wide"
												>
													<div
														data-ui="detail-label"
														className="bo-tableBookingDetailLabel"
													...
													<div
														data-ui="detail-value"
														className="bo-tableBookingDetailValue bo-tableBookingDetailValue--wrap"
													>
														{selectedBooking.commentary}
													</div>
												</div>
										...
										<div
											data-ui="field-booking-tables"
											className="bo-tableBookingTableField"
										>
											<label
												data-ui="booking-tables-label"
												className="bo-tableBookingTableLabel"
											>
												Mesas asignadas
											</label>
											{visibleTables.length === 0 ? (
												<div
													data-ui="booking-table-empty"
													className="bo-tableBookingTableEmpty"
													role="status"
												>
													No hay mesas creadas, por favor primero crea una mesa
													nueva.
												</div>
											) : (
												<BookingAssignmentEditor
													...
													occupiedSeats={occupiedSeatsExcludingBooking(
														selectedBooking,
													)}
													onSave={(assignments) =>
														void saveBookingAssignments(
															selectedBooking,
															assignments,
														)
													}
										...
										<div
											data-ui="booking-modal-actions"
											className="bo-tableBookingModalActions"
										>
											<button
												data-ui="cancel-booking-btn"
												className="bo-btn bo-btn--ghost bo-btn--sm"
												type="button"
											...
											<div
												data-slot="actions-right"
												className="bo-tableBookingActionsRight"
											>
												{visibleTables.length > 0 ? (
													<button
							...
							<Modal
								...
								widthPx={480}
								className="bo-tableRemoveAreaModal"
								hideClose
							>
								<ModalHeader
									data-slot="modal-head"
								...
								/>
								<div
									data-ui="delete-table-confirm"
									className="bo-tableRemoveAreaConfirm"
								>
									<p
										data-ui="delete-table-message"
										className="bo-tableRemoveAreaText"
									>
										Se eliminara la mesa{" "}
										<strong>
											{visibleTables.find((t) => t.id === deleteConfirmTableId)
												?.name || `#${deleteConfirmTableId ?? ""}`}
										</strong>
										. Esta accion no se puede deshacer.
									</p>
									<div
										data-ui="delete-table-actions"
										className="bo-modalActions"
									>
										<button
											data-ui="cancel-delete-table-btn"
											className="bo-btn bo-btn--ghost"
											type="button"
										...
										<button
											data-ui="confirm-delete-table-btn"
							...
							<Modal
								open={removeAreaConfirmOpen}
								title="Eliminar area"
								onClose={() => setRemoveAreaConfirmOpen(false)}
								widthPx={480}
								className="bo-tableRemoveAreaModal"
								hideClose
							>
								...
								/>
								<div
									data-ui="remove-area-confirm"
									className="bo-tableRemoveAreaConfirm"
								>
									<p
										data-ui="remove-area-message"
										className="bo-tableRemoveAreaText"
									>
										Se eliminaran los limites del mapa y{" "}
										<strong>todos los elementos</strong> dibujados dentro de el
										(muros, obstaculos, imagenes). Las mesas se mantendran.
									</p>
									<div
										data-ui="remove-area-actions"
										className="bo-modalActions"
									>
										<button
											data-ui="cancel-remove-area-btn"
											className="bo-btn bo-btn--ghost"
											type="button"
											onClick={() => setRemoveAreaConfirmOpen(false)}
										>
											Cancelar
										</button>
										<button
											data-ui="confirm-remove-area-btn"
											className="bo-btn bo-btn--danger"
											type="button"
											onClick={removeArea}
										>
							...
							{multiTableNamesModalIdx !== null &&
								multiTableDraft[multiTableNamesModalIdx] && (
									<GuestNamesModal
										tableName={
											multiTableDraft[multiTableNamesModalIdx].table_name
										}
