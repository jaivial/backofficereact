...
import type { APIError, CalendarDay, ConfigDailyLimit, DashboardMetrics } from "../../../api/types";
...
export async function data(pageContext: PageContextServer) {
  ...
  const safeCall = async <T>(promise: Promise<T>, message: string): Promise<T | APIError> => {
    try {
  ...
  const [bookingsRes, calRes, limitRes, metricsRes] = await Promise.all([
    safeCall(api.reservas.list({ date, page: 1, count: 15, sort: "reservation_time", dir: "asc" }), "Error consultando reservas"),
    safeCall(
      (() => {
    ...
    safeCall(api.config.getDailyLimit(date), "Error consultando límite diario"),
    safeCall(api.dashboard.getMetrics(date), "Error consultando métricas"),
  ]);
  ...
  const calendarDays: CalendarDay[] = calRes.success ? calRes.data : [];
  const dailyLimit: ConfigDailyLimit | null = limitRes.success ? limitRes : null;
  const metrics: DashboardMetrics | null = metricsRes.success ? metricsRes.metrics : null;
