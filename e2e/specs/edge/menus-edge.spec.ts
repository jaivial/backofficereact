...
test.describe("@edge Menus", () => {
  test("paneles de tipo de menu visibles con contadores", async ({ adminPage }) => {
    ...
    await expect(adminPage.getByTestId("menus-page-section")).toBeVisible({ timeout: 15_000 });
  ...
  test("elegir tipo muestra tarjetas y volver funciona", async ({ adminPage }) => {
    ...
    const cards = adminPage.locator('[data-testid^="menu-summary-"]').filter({ hasNot: adminPage.locator('[data-testid$="-edit-"], [data-testid$="-delete-"], [data-testid$="-change-type-"]') });
  ...
  test("busqueda por titulo filtra tarjetas", async ({ adminPage }) => {
    ...
    const after = await adminPage.locator('[data-testid^="menu-summary-"]').count();
  ...
  test("crear menu abre modal de plantillas y se cierra", async ({ adminPage }) => {
    ...
    // Modal con selección de plantilla
