	...
	return page.evaluate(
		([type2, x2, y2]) => {
			const node = (document.querySelector(
				'[data-ui="table-node"].is-selected',
...
async function seedTablePositions(api: TestApiClient) {
	...
	const list = (await api.get(
		`/api/admin/tables?date=${TEST_DATE}&floor_number=0`,
	)) as {
...
test.describe("Tables Map - Table resize (touch)", () => {
	...
	test("touch drag on corner handle grows the table continuously", async ({
		adminTouchPage,
	}) => {
		...
		const end = {
			x: handleCenter.x + 80 * zoom,
			y: handleCenter.y + 60 * zoom,
		};
	...
	test("desktop mouse drag on the same handle still resizes (regression)", async ({
		adminPage,
	}) => {
		...
		let lastWrapper = startState.wrapper;
		for (let i = 1; i <= 8; i += 1) {
			await page.mouse.move(h.x + (90 * i) / 8, h.y + (70 * i) / 8, {
				steps: 2,
			});
